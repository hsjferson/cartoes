...
public class TransacaoControllerTest {
    ...
    private List<Transacao> CriarTransacaoTestes() throws ParseException {
        ...
        clienteTeste.setCpf("12349209418");
        clienteTeste.setNome("Davi Arndt");
        clienteTeste.setUf("PR");
        clienteTeste.setDataAtualizacao(new SimpleDateFormat("dd/MM/yyyy").parse("14/09/2020"));

        cartaoTeste.setNumero("12345678910");
        cartaoTeste.setBloqueado(false);
        cartaoTeste.setDataValidade(new SimpleDateFormat("dd/MM/yyyy").parse("11/12/2022"));
        ...
        transacaoTeste.setDataTransacao(new SimpleDateFormat("dd/MM/yyyy").parse("27/08/2020"));
    ...
    public void testSalvarCnpjEmBranco() throws Exception {
        ...
        objEntrada.setNumeroCartao("8888777711114");
    ...
    public void testSalvarJurosInvalido() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.post("/api/transacao").content(json).contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)).andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.erros[0]").value("Juros precisa ser preenchido com no máximo 4 caracteres numéricos."));
    ...
    public void testSalvarQtdParcelasInvalido() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.post("/api/transacao").content(json).contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)).andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.erros[0]").value("Quantidade de Parcelas precisa ser preenchido com no máximo 2 caracteres numéricos."));
    ...
    public void testSalvarValorInvalido() throws Exception {
        ...
        mvc.perform(MockMvcRequestBuilders.post("/api/transacao").content(json).contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)).andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.erros[0]").value("Valor precisa ser preenchido com no máximo 10 caracteres numéricos."));
